-- ScalaShield 2026 - Phase 1 schema
-- Covers data model 5.2 (Framework) and 5.3 (Threat) as live Quill-mapped
-- tables. Tables for 5.4-5.9 are created now so later phases can add Flyway
...
-- 5.4 ThreatTranslation (i18n, Phase 5) - table only, not Quill-mapped yet
...
-- 5.6 Mitigation (Phase 2) - table only, not Quill-mapped yet
...
-- 5.7 CodeSample (Phase 3) - table only, not Quill-mapped yet
...
-- 5.8 CrossReference (Phase 2+) - table only, not Quill-mapped yet
...
-- 5.5 CornucopiaCard (Phase 6+) - table only, not Quill-mapped yet
...
-- 5.9 ContentHash (Phase 6+, SSDLC) - table only, not Quill-mapped yet
