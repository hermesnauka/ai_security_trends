# SharpGuard 2026 — User Stories & TDD Test Plan

**Version:** 1.0
**Date:** 2026-07-07
**Stack:** C# / .NET 9 (ASP.NET Core Minimal API + EF Core, backend) · React 18 + TypeScript + Vite (frontend)
**Test frameworks:** `xUnit` + `FluentAssertions` + `FsCheck` (property-based) + `WebApplicationFactory<T>` + `NetArchTest` · Vitest + React Testing Library · Playwright

---

## Konwencje testowe

### Backend (C# — `xUnit` + `FluentAssertions`)
```csharp
public class ThreatServiceTests
{
    ...
    public async Task FiltersBySeverity()
    {
        ...
        var result = await new ThreatService(env.DbContext).ListAsync(new ThreatFilter(Severity: Severity.Critical));
        result.Should().OnlyContain(t => t.Severity == Severity.Critical);
    }
}
```

### Backend properties (`FsCheck`)
```csharp
public class CardLoaderPropertyTests
{
    [Property]
    public Property RejectsUnknownTopLevelFields(NonEmptyString extraKey)
    {
        var yaml = $"meta:\n  edition: webapp\n{extraKey.Get}: evil\nsuits: []\n";
        return Prop.ForAll(() =>
        {
            Action act = () => CardLoader.Decode(yaml);
...
### Backend integration (`WebApplicationFactory<T>`)
```csharp
public class ThreatEndpointsTests : IClassFixture<WebApplicationFactory<Program>>
{
    private readonly HttpClient _client;
    public ThreatEndpointsTests(WebApplicationFactory<Program> factory) => _client = factory.CreateClient();

    [Fact]
    public async Task ListReturns200ForValidFramework()
    {
        var response = await _client.GetAsync("/api/v1/threats?framework=OWASP_LLM");
...
{
    [Fact]
    public void EndpointsDoNotDependOnIntegrity()
    {
        var result = Types.InAssembly(typeof(Program).Assembly)
            .That().ResideInNamespace("SharpGuard.Api.Endpoints")
            .ShouldNot().HaveDependencyOn("SharpGuard.Core.Integrity")
            .GetResult();
...
```
RED       — napisz test (xUnit, właściwość FsCheck, lub test architektury NetArchTest) opisujący
             oczekiwane zachowanie PRZED napisaniem kodu produkcyjnego
GREEN     — napisz minimalny kod, który sprawia, że test przechodzi
REFACTOR  — poprawiaj kod bez łamania testów; kompilator (nullable, CS8509-jako-błąd) i Roslyn
             analyzery pilnują jakości w tle na każdym `dotnet build`
```
...
```csharp
[Fact]
public async Task List_ReturnsAtLeastTenSeededFrameworks()
{
    using var env = TestEnv.Create();
    var frameworks = await new FrameworkService(env.DbContext).ListAsync();
    frameworks.Count.Should().BeGreaterOrEqualTo(10);
}

[Fact]
public async Task Detail_404sForUnknownCode()
{
    var response = await _client.GetAsync("/api/v1/frameworks/NOT_REAL");
...
```csharp
[Fact]
public async Task CombinesFrameworkAndSeverityFilters()
{
    using var env = TestEnv.Create();
    var result = await new ThreatService(env.DbContext).ListAsync(
        new ThreatFilter(FrameworkCode: "OWASP_LLM", Severity: Severity.Critical));
    result.Should().OnlyContain(t => t.FrameworkCode == "OWASP_LLM" && t.Severity == Severity.Critical);
}

[Fact]
public async Task RejectsQueryLongerThan200Chars()
{
    var response = await _client.GetAsync($"/api/v1/threats?q={new string('a', 201)}");
...
```csharp
[Property]
public Property EverySeededMitigationHasAllFiveLanguages()
{
    return Prop.ForAll(SeededMitigationGen.Instance, mitigation =>
    {
        var languages = mitigation.CodeSamples.Select(s => s.Language).Distinct().OrderBy(l => l).ToList();
        return languages.SequenceEqual(new[] { CodeLanguage.Go, CodeLanguage.Java, CodeLanguage.Lua, CodeLanguage.Python, CodeLanguage.Scala }.OrderBy(l => l));
    });
}
```
...
```csharp
[Fact]
public async Task Llm01MapsToAmlT0051()
{
    using var env = TestEnv.Create();
    var refs = await new CrossReferenceService(env.DbContext).BySourceCodeAsync("LLM01");
    refs.Select(r => r.TargetCode).Should().Contain("AML.T0051");
}
```
...
```csharp
[Fact]
public async Task FreCards_EachCarryAtLeastOneOwaspRef()
{
    using var env = TestEnv.Create();
    var cards = await new CardService(env.DbContext).BySuitAsync("FRE");
    cards.Should().OnlyContain(c => c.OwaspRefs.Any());
}

[Fact]
public async Task Fre4_PolishTranslationIsReviewedNotIdenticalToEnglish()
{
    using var env = TestEnv.Create();
    var card = await new CardService(env.DbContext).ByCardIdAsync("FRE4", Locale.Pl);
    card.DescriptionPl.Should().NotBe(card.DescriptionEn);
    card.DescriptionPl.Should().Contain("James wstrzykuje");
}
```
...
```csharp
[Fact]
public async Task LlmMatrix_MapsLlm10ToCardLlm2()
{
    using var env = TestEnv.Create();
    var matrix = await new MatrixService(env.DbContext).LlmAsync();
    matrix.RowsFor("LLM10").Select(r => r.CardId).Should().Contain("LLM2");
}
```
...
```csharp
[Fact]
public async Task AaiK_IsFlaggedCritical()
{
    using var env = TestEnv.Create();
    var card = await new CardService(env.DbContext).ByCardIdAsync("AAIK", Locale.En);
    card.IsCritical.Should().BeTrue();
}

[Fact]
public async Task Cld3_CrossReferencesA05()
{
    using var env = TestEnv.Create();
    var card = await new CardService(env.DbContext).ByCardIdAsync("CLD3", Locale.En);
    card.OwaspRefs.Select(r => r.Value).Should().Contain("A05:2021");
}
```
...
```csharp
[Fact]
public async Task Heatmap_ReturnsExactlySixCategories()
{
    using var env = TestEnv.Create();
    var heatmap = await new StrideHeatmapService(env.DbContext).ComputeAsync();
    heatmap.Categories.Should().HaveCount(6);
}

[Fact]
public async Task StrideHeatmap_RequiresAuth()
{
    var response = await _client.GetAsync("/api/v1/stride-heatmap");
...
```csharp
[Fact]
public async Task Emr2_HasNoOwaspRefButHasMiscNote()
{
    using var env = TestEnv.Create();
    var card = await new CardService(env.DbContext).ByCardIdAsync("EMR2", Locale.En);
    card.OwaspRefs.Should().BeEmpty();
    card.MiscNote.Should().NotBeNull();
}
```
...
```csharp
[Fact]
public async Task MobileVsWeb_MapsMasvsStorage()
{
    using var env = TestEnv.Create();
    var matrix = await new MatrixService(env.DbContext).MobileVsWebAsync();
    matrix.RowsFor("MASVS-STORAGE").Should().NotBeEmpty();
}
```
...
```csharp
[Fact]
public async Task BotSuit_Returns429After60Requests()
{
    for (var i = 0; i < 60; i++)
    {
        await _client.GetAsync("/api/v1/threats?suit=bot");
    }
    var response = await _client.GetAsync("/api/v1/threats?suit=bot");
    response.StatusCode.Should().Be((HttpStatusCode)429);
    response.Headers.Should().ContainKey("Retry-After");
}

[Fact]
public async Task DvoK_CrossReferencesA06()
{
    using var env = TestEnv.Create();
    var card = await new CardService(env.DbContext).ByCardIdAsync("DVOK", Locale.En);
    card.OwaspRefs.Select(r => r.Value).Should().Contain("A06:2021");
}
```
...
```csharp
[Fact]
public async Task WebsiteApp_HasExactlySixSuits()
{
    using var env = TestEnv.Create();
    var suits = await new CardService(env.DbContext).SuitsByEditionAsync(Edition.Webapp);
    suits.OrderBy(s => s).Should().BeEquivalentTo(new[] { "AT", "AZ", "C", "CR", "SM", "VE" });
}

[Fact]
public async Task Ve3_CrossReferencesA03()
{
    using var env = TestEnv.Create();
    var card = await new CardService(env.DbContext).ByCardIdAsync("VE3", Locale.En);
    card.OwaspRefs.Select(r => r.Value).Should().Contain("A03:2021");
}
```
...
```csharp
[Property]
public Property EverySeededMitigationHasAPythonSample()
{
    return Prop.ForAll(SeededMitigationGen.Instance, mitigation =>
        mitigation.CodeSamples.Any(s => s.Language == CodeLanguage.Python));
}
```
...
**Cel:** porównać z idiomami C#/EF Core tej aplikacji
...
```csharp
[Fact]
public async Task Llm2Mitigation_HasLuaRestyLimitReqSample()
{
    using var env = TestEnv.Create();
    var samples = await new CodeSampleService(env.DbContext).ByMitigationIdAsync(Llm2MitigationId());
    var lua = samples.Single(s => s.Language == CodeLanguage.Lua);
    lua.Code.Should().Contain("lua-resty-limit-req");
}
```
...
```csharp
[Fact]
public async Task Search_ReturnsHighlightedExcerpts()
{
    using var env = TestEnv.Create();
    var results = await new SearchService(env.DbContext).QueryAsync("deepfake", Locale.En);
    results.Should().NotBeEmpty();
    results[0].Excerpt.Should().Contain("<mark>");
}

[Fact]
public async Task Search_RejectsQueryOver200Chars()
{
    var response = await _client.GetAsync($"/api/v1/search?q={new string('a', 201)}");
    response.StatusCode.Should().Be(HttpStatusCode.UnprocessableEntity);
}
```
...
```csharp
[Fact]
public async Task Export_EnqueuesJobAndReturns202()
{
    var response = await _client.GetAsync("/api/v1/export?format=csv&framework=OWASP_LLM");
    response.StatusCode.Should().Be(HttpStatusCode.Accepted);
    var body = await response.Content.ReadFromJsonAsync<ExportJobResponse>();
    body!.JobId.Should().NotBeNullOrEmpty();
}

[Fact]
public void ExportCsvArgs_RejectsMissingFramework()
{
    var args = new ExportCsvArgs(Format: "csv", Framework: "");
    ExportArgsValidator.Validate(args).IsValid.Should().BeFalse(); // AC-14
}
```
...
**Ważna uwaga metodologiczna:** ta talia — w przeciwieństwie do FRE/LLM/AAI/CLD/STRIDE/MLSec/Mobile/DVO/BOT/Website App — **nie opisuje podatności technicznych z określonym poziomem severity**. W modelu danych C# karta `dbd` ma `Kind` będące instancją `DesignHarm` — sealed record **bez pola** severity (patrz `PLAN.md` D-04). Wyrażenie `switch` bez wyczerpującej obsługi obu wariantów `CardKind` generuje ostrzeżenie `CS8509`, które w tym projekcie jest skonfigurowane jako błąd budowania. Poniższy test dokumentuje kształt serializacji JSON, jedyną obserwowalną konsekwencję tej gwarancji.
...
```csharp
[Fact]
public void DesignHarmVariant_HasNoSeverityField()
{
    CardKind kind = new DesignHarm();
    var severity = CardKindExtensions.SeverityOf(kind); // exhaustive switch, CS8509-as-error
    severity.Should().BeNull();
}

[Fact]
public async Task Sco2_CrossReferencesA04()
{
    using var env = TestEnv.Create();
    var card = await new CardService(env.DbContext).ByCardIdAsync("SCO2", Locale.En);
    card.OwaspRefs.Select(r => r.Value).Should().Contain("A04:2021");
}

[Fact]
public async Task DigitalHarmsSuits_NeverReturnASeverityValue()
{
    var response = await _client.GetAsync("/api/v1/threats?suit=sco");
    var body = await response.Content.ReadFromJsonAsync<JsonElement>();
    foreach (var item in body.GetProperty("items").EnumerateArray())
    {
        item.GetProperty("cardKind").GetProperty("kind").GetString().Should().Be("DesignHarm");
        item.GetProperty("cardKind").TryGetProperty("severity", out _).Should().BeFalse();
    }
}
```
...
test.describe('US-19 Digital-by-Default Harms', () => {
  ...
  test('API response for dbd cards never carries a severity value', async ({ request }) => {
    ...
    for (const card of body.items) {
      expect(card.cardKind.kind).toBe('DesignHarm')
      expect(card.cardKind.severity).toBeUndefined()
    }
  })
})
```
...
|---|---|---|---|
| Backend — unit | `xUnit` + `FluentAssertions` | ≥ 55 | Serwisy, typy, konstruktory, serializacja JSON |
| Backend — właściwości | `FsCheck` | ≥ 15 | Niezmienniki (5 języków per mitigacja, YAML z nieznanymi polami odrzucany) |
| Backend — integracja | `WebApplicationFactory<T>` + testowa PostgreSQL (Docker) | ≥ 26 | Endpointy, EF Core, rate-limit |
| Backend — architektura | `NetArchTest` | ≥ 3 | Granica `Endpoints` ↔ `Integrity` (D-05) |
...
|---|---|---|
| Backend C# | `coverlet` | ≥ 85% dla `SharpGuard.Core/Services` |
| Frontend React | Vitest v8 coverage | ≥ 75% |

### Kolejność wykonania testów w CI

```
[1] dotnet build -warnaserror                — kompilacja (nullable + CS8509 jako błędy)
[2] dotnet test SharpGuard.Tests/Unit         — xUnit + FsCheck (bez bazy danych)
[3] dotnet test SharpGuard.Tests/Integration  — WebApplicationFactory + testowa PostgreSQL
[4] dotnet test SharpGuard.Tests/Architecture — NetArchTest
[5] dotnet list package --vulnerable          — SCA
[6] dotnet publish -p:PublishAot=true          — weryfikacja kompatybilności Native AOT
...
|---|---|---|
| AC-01: SQL Injection `?q=` | EF Core parametryzacja — test integracyjny | Sprint 3–4 |
| AC-02: XSS w opisie karty | `HtmlSanitizer`/DOMPurify integration test | Sprint 5–7 |
| AC-05: Bot scraping | `WebApplicationFactory` — 429 po 60 req | Sprint 10–12 |
| AC-06: YAML file tampering | `Integrity.Verify()` — abort na hash mismatch | Sprint 5–7 |
| AC-09: Privilege escalation `/stride-heatmap` | JWT policy — 401 bez roli | Sprint 5–7 |
| AC-11: i18n locale injection | `Locale` enum — parsowanie nieznanej wartości zwraca `En` | Sprint 9–10 |
| AC-14: Malicious Hangfire job payload | Silnie typowane parametry zadania odrzucają niedopasowany kształt | Sprint 10–12 |
| AC-15: BotWarningModal bypass | `us11-devops-security.spec.ts` — bezpośrednie wywołanie API wciąż zablokowane | Sprint 5–7 |
| AC-16: Harms deck misread as CVE severity | Test serializacji JSON — brak wartości severity dla `DesignHarm` | Sprint 5–7 |
| AC-17: Konfiguracja `CS8509`-jako-błąd usunięta | CI grep sprawdzający każdy `.csproj` | Sprint 1–2 (weryfikowane co sprint) |
