...
DB_NAME="${RUBYGUARD_DB_NAME:-rubyguard_development}"
DB_ROLE="${RUBYGUARD_DB_ROLE:-rubyguard}"

if ! psql -U postgres -tAc "SELECT 1 FROM pg_roles WHERE rolname='${DB_ROLE}'" | grep -q 1; then
  psql -U postgres -c "CREATE ROLE ${DB_ROLE} WITH LOGIN PASSWORD 'rubyguard';"
fi
...
echo "rubyguard: role '${DB_ROLE}' and database '${DB_NAME}' ready on the shared local Postgres instance."
echo "Next: cd backend && bundle install && bundle exec rake db:migrate db:seed"
