...
**Provenance is stated explicitly per requirement below, per `PLAN.md` §0.1:** the source mapping document fully covers OWASP LLM Top 10, OWASP Web Top 10, MITRE ATLAS, CompTIA Security+/SecAI+, and a STRIDE↔OWASP mapping; it does **not** cover Agentic AI Top 10, API Security Top 10, Client-Side Top 10, CI/CD Security Top 10, OAT, or MASVS, and the raw Cornucopia YAML files carry no `severity`/`card_kind`/cross-reference fields at all. Every requirement below that depends on data not present in a source file says so, and names the curation mechanism (a reviewed `curation/*.json` file, `PLAN.md` D-08) that supplies it instead. This document is also explicit about where its enforcement mechanisms are weaker than a custom-backend sibling's — and about the one place (the Digital-by-Default Harms deck's data-layer `CHECK` constraint, FR-19.2) where this project's guarantee is arguably stronger than any sibling's.
...
- FR-02.4 Filtering shall debounce at 300 ms and update results via `fetch()` against the REST API without a full page reload; the equivalent filtered results shall also be reachable via standard query-string URL parameters that work with JavaScript disabled (progressive enhancement).
...
- FR-03.2 Every mitigation shall list at least one code sample per language (Python, Java, Go, Scala, Lua) — **original content authored for this project** (`PLAN.md` §0.1: no source file supplies multi-language code samples), verified by an `eris` property test and a CI seed-data linter.
...
- FR-04.1 A `/matrix/` template shall map threats across frameworks (at minimum OWASP LLM Top 10 ↔ MITRE ATLAS ↔ CompTIA SecAI+ — the one cross-framework mapping the source document states explicitly, e.g. LLM01 → AML.T0051).
...
- FR-05.2 Cards shall show card ID, value, suit, PL/EN-selectable description, and OWASP Client-Side Top 10 reference chips — the chip value is **curated content** (`curation/companion.curation.json`), since the raw YAML carries no such field.
...
- FR-06.2 `/matrix/llm/` shall map OWASP LLM Top 10 (2025, fully described in the source document) entries to Cornucopia `LLM` cards, indicating coverage gaps with an empty dashed cell.
...
- FR-07.2 `/matrix/agentic/` shall compare OWASP Agentic AI Top 10 (2026 — **curated/authored**, `PLAN.md` §11: not present in either source file) with OWASP LLM Top 10 (2025, source-documented).
...
- FR-08.1 `/frameworks/stride/` shall display all STRIDE EoP v5.0 cards, grouped by the 6 STRIDE suits.
...
- FR-09.1 `/frameworks/ml-security/` shall display all MLSec v1.0 cards (EMR/EIR/EOR/EDR).
- FR-09.2 Cards shall show MITRE ATLAS reference chips (curated, e.g. `EMR2` → a maintainer-assigned technique ID, since the raw YAML has no MITRE-reference field); adversarial-ML/model-extraction cards shall carry an `ML-SPECIFIC` badge.
...
- FR-10.2 `/matrix/mobile-vs-web/` shall compare MASVS 2.0 categories (**curated/authored** — not present in either source) with OWASP Web Top 10 (source-documented).

### FR-11 — Cornucopia: DevOps + Cloud + BOT Security, and WordPress-specific hardening (US-11)
- FR-11.1 `/frameworks/devops-security/` shall contain a DVO section (CI/CD Security Top 10 chips — curated/authored), a CLD section (A05:2021/A01:2021 chips, source-documented), and a BOT section (OAT chips — curated/authored).
...
- FR-11.5 This story additionally covers hardening the WordPress installation itself as a worked example: `xmlrpc.php` disabled, login-attempt rate limiting on `wp-login.php`, `DISALLOW_FILE_EDIT` set, and the WPScan-flagged surface documented in `SDLC_analysis.md` — presented on the DevOps page as a "this very site" case study, since neither source document contains WordPress-specific content at all and this is entirely original material.
...
- FR-12.2 Cards shall cross-reference the OWASP Web Top 10 entry they correspond to (e.g. `VE3` → `A03:2021`) — this mapping is curated, since the raw YAML has no such field, but the target OWASP Web Top 10 entries themselves are source-documented.

### FR-13 — Python Code Samples (US-13)
- FR-13.1 Every mitigation with a code sample shall include a Python tab (Django ORM or FastAPI + Pydantic idioms) — original authored content.

### FR-14 — Java Code Samples (US-14)
- FR-14.1 Every mitigation with a code sample shall include a Java tab (Spring Boot 3.3, Spring Security 6, Spring Data JPA) — original authored content, distinct from the one Java/LangChain4j snippet found in the source document (that snippet demonstrates LLM guardrails, not this plugin's own mitigation catalogue, and is not reused verbatim).
...
- FR-15.1 Every mitigation with a code sample shall include a Scala tab (Akka HTTP/http4s, Slick/Doobie, ZIO 2 idioms) — original authored content.

### FR-16 — Lua Code Samples (US-16)
- FR-16.1 Every mitigation with a code sample shall include a Lua tab (OpenResty/NGINX Lua idioms) — original authored content.
...
### FR-18 — i18n: Polish ↔ English, via WordPress's native mechanism, with a visible switch
- FR-18.1 The application shall support Polish and English locales only; Polish is this plugin's shipped default configuration.
- FR-18.2 A **visible language-switch control**, present in the header of every template (rendered by `language-toggle.js` where JS is available, degrading to a plain `<a href="?lang=en">`/`<a href="?lang=pl">` link pair otherwise), shall let the user change PL→EN and EN→PL at will. It sets a `?lang=` query variable (or, for logged-in users, their WordPress user-locale preference via `get_user_locale()`), re-fetching translated content without a full page reload where feasible.
...
- FR-18.4 Threat/card titles, descriptions, and attack-vector text (distinct from FR-18.3's UI strings) shall be served in the requested locale from `sp_threat_translations`/`sp_cards.description_pl`/`description_en`.
...
- FR-18.7 CI shall run `wp i18n make-pot` and fail the build if the generated `.pot` file differs from the committed one.
...
- FR-19.2 Each card shall render with a design-harm badge, never the severity badge used for technical threats. Since the raw YAML has **no field distinguishing design-harm from technical-threat cards at all** (`PLAN.md` §0.1), `card_kind = 'design_harm'` is assigned to every card ingested from `dbd-cards-1.0-en.yaml` by the loader itself (not per-card curation), and enforced at two independent layers: (a) the `sp_cards.card_kind` column plus the `chk_design_harm_has_no_severity` MySQL `CHECK` constraint (`PLAN.md` D-04), rejecting any row where `card_kind = 'design_harm'` and `severity` is non-NULL **at the database engine level**; and (b) the REST API response omitting the `severity` field entirely when `card_kind = 'design_harm'`, verified by an integration test.
- FR-19.3 Each card shall display a cross-reference chip linking to OWASP A04:2021 Insecure Design (curated), and, where relevant, to the CompTIA SecAI+ GRC/AI-Act topic list (source-documented topic bucket E, `PLAN.md` §0.1).
...
- SR-02.1 All database access goes through `$wpdb->prepare()` with `%s`/`%d`/`%f` placeholders (`PLAN.md` D-02). This gives no compile-time guarantee — PHP has no mechanism equivalent to `sqlc`/`hasql-th`/`sqlx::query!`. The guarantee here is entirely: (a) the `WordPress.DB.PreparedSQL` WPCS sniff pattern-matching likely-unprepared queries, and (b) code review.
...
- SR-03.3 Admin-edited threat/mitigation text is passed through `wp_kses()` with an explicit allow-list (`b`, `i`, `code`, `pre`, `a[href]`) before persisting, in addition to output-time escaping.
...
- SR-04.1 A response-header filter (`send_headers` action) injects on every response: CSP, `Strict-Transport-Security` (max-age ≥ 31536000, includeSubDomains), `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy`.
...
- SR-05.4 `wp-login.php` login attempts are separately rate-limited (a well-known WordPress hardening requirement, included because this app is also a worked example of securing the WordPress installation it runs on, per FR-11.5).
...
- SR-06.2 On any SHA-256 mismatch against `data/hashes.json`, the calling code aborts — fail-secure, no partial writes (a single `$wpdb` transaction).
- SR-06.3 `data/hashes.json` covers the six raw YAML source files only; `curation/*.curation.json` files are versioned in git and reviewed via normal PR diff instead (`PLAN.md` §12), since they are original content, not third-party source material requiring tamper-detection.
- SR-06.4 `Integrity_Verifier::verify()` is called from no code under `includes/rest-api/`, verified by a custom PHPStan rule and code review (`PLAN.md` D-03) — no process boundary and no compiler-enforced module visibility exist here to fall back on.

### SR-07 — Reference ID Allowlists
- SR-07.1 `OwaspRef`, `MitreRef`, `MasvsRef`, `CicdSecRef`, `OatRef` values — every one of them curated content per `PLAN.md` §0.1/§4 D-08, since none exists in the raw YAML — are validated against allowlists loaded from `data/ref-allowlists.json`/`data/mitre-atlas-allowlist.json` inside a small `Reference_Validator` class before any write to `owasp_refs`/`mitre_refs` JSON columns.
- SR-07.2 Any admin CRUD request supplying an unknown reference ID is rejected with HTTP 422 / a `WP_Error`; the same validator runs against `curation/*.curation.json` files in CI (`PLAN.md` §12).
...
- SR-08.2 This requirement is stated as stronger, in this one specific and narrow respect, than any prior sibling's equivalent precisely because those all depend on the application's own code being the only path to the data, while a database `CHECK` constraint holds against any client of the database, including future code this plugin's authors have not yet written.

### SR-09 — YAML Parsing & Curation-File Safety
- SR-09.1 All six Cornucopia YAML decks are decoded via `symfony/yaml`'s `Yaml::parseFile()` (never with custom-tag/object-support flags enabled) followed by hand-written allow-list key validation against a `CardFile` shape — the raw shape is only `id`/`value`/`url`/`desc`/`misc` (`PLAN.md` §0.1); any additional key is rejected.
- SR-09.2 Curation files (`curation/*.curation.json`) are decoded via PHP's native `json_decode()`, validated against their own allow-list schema (`card_id`, `severity`, `card_kind`, `owasp_refs`, `mitre_refs` only), and merged with the raw card by `card_id` — a card present in the curation file but absent from the raw YAML (or vice versa) fails ingestion rather than being silently dropped or silently left uncurated.
- SR-09.3 An `eris` property test generates YAML/JSON documents with random extra keys and asserts rejection for both the raw-card and curation-file decoders.
...
- SR-10.4 The WPScan vulnerability database is scanned against this plugin, the WordPress core version, and every other plugin/theme in the deployment on every PR and nightly.
- SR-10.5 `npm audit --audit-level=high` fails the build tooling's own dependency check on HIGH/CRITICAL advisories.
- SR-10.6 Trivy scans the WordPress/PHP-FPM container image; fails on HIGH/CRITICAL.

### SR-11 — CSRF
- SR-11.1 Every state-changing form and AJAX call uses WordPress nonces (SR-01.4); this is the platform's native CSRF defense and is used exclusively.
...
### SR-14 — WordPress-Specific Platform Hardening
...
- SR-14.3 WordPress core, this plugin, and all Composer dependencies are updated via a controlled, staging-first process — not WordPress's default unattended auto-update posture.
- SR-14.4 The WordPress REST API's user-enumeration-via-author-archive behavior is disabled or rate-limited.
...
### NFR-01 — Performance
- NFR-01.1 API endpoints respond in ≤ 250 ms (p95) under 50 concurrent users.
- NFR-01.2 MySQL `FULLTEXT` search returns results in ≤ 400 ms (p95).
- NFR-01.3 The vanilla-JS bundle (all modules combined, minified) is ≤ 100 KB gzip.
...
- NFR-02.4 All core flows are fully keyboard-navigable, and, per FR-02.4/FR-03.1, fully functional with JavaScript disabled.
...
- NFR-03.4 The plugin is versioned independently of WordPress core and of any theme; activating it on a fresh WordPress install with any standard theme active must fully function, verified by a CI test matrix against at least two different default WordPress themes.

### NFR-04 — Testability (TDD — see `user_stories+tests.md`)
- NFR-04.1 `includes/service` code coverage ≥ 85%.
- NFR-04.2 Every REST controller has a `WP_UnitTestCase`-based integration test.
- NFR-04.3 Every untrusted-input-parsing function (raw-YAML decoder, curation-file decoder, reference validators) shall have at least one `eris` property test in addition to example-based PHPUnit cases.
...
- NFR-05.2 The plugin activates cleanly on a standard WordPress 6.8+ install without requiring any non-standard server configuration beyond PHP 8.3 and MySQL 8.0.16+.
- NFR-05.3 Static assets (CSS, JS, Prism.js grammars) are self-hosted via `wp_enqueue_*` — no external CDN dependency.
- NFR-05.4 WP-Cron is disabled from its default page-view-triggered mode and driven by a real system crontab entry hitting `wp-cron.php` on a fixed schedule.

### NFR-06 — Internationalization Quality
- NFR-06.1 Language switching completes in under 150 ms perceived latency where the client-side enhancement path is used; a full page navigation (no-JS fallback) is acceptable and expected to be slower, by design.
...
- NFR-07.1 A WordPress installation's footprint is dominated by WordPress core itself, not by this plugin. The plugin's own code footprint (excluding WordPress core, Composer dependencies, and seed data) targets ≤ 5 MB.
...
- C-07 Only Polish and English are supported; both `.po`/`.mo` files must be complete before a new translation string reaches production, and a visible switch between them is present on every page (FR-18.2).
- C-08 `CornucopiaCard` rows (all six decks) are never editable through any UI or API — no such code exists in the plugin at all (`PLAN.md` D-07). Content changes only through a reviewed PR to the source YAML and/or curation files plus a hash-allowlist update.
- C-09 The Redis instance backing the object cache/rate limiter is never exposed on a public port.
- C-10 The plugin's functionality shall not depend on any specific WordPress theme being active (NFR-03.4).
...
- C-12 No `severity`, `card_kind`, or OWASP/MITRE cross-reference value is ever read directly from a raw `docs/OWASP_stories/*.yaml` file at runtime — these fields exist only in `data/cornucopia/curation/*.curation.json`, since the raw source has none (`PLAN.md` §0.1). This is a hard constraint on the ingestion pipeline, not an implementation detail.
...
- DR-01.1 All 10 OWASP Web Top 10 (2021) entries with mitigations and 5-language code samples — source-documented.
- DR-01.2 All 10 OWASP LLM Top 10 (2025) entries with mitigations and 5-language code samples — source-documented.
- DR-01.3 Minimum 15 MITRE ATLAS techniques (≥ 5 tactics) — a superset of the techniques named in the source document (AML.T0010, T0020, T0024, T0029, T0043, T0051) extended with curated additions to reach the minimum.
- DR-01.4 All 10 OWASP Agentic AI Top 10 (2026) entries — curated/authored (not in either source).
- DR-01.5 Minimum 20 CompTIA SecAI+/Security+ SY0-701 topics, drawn from the source document's topic buckets plus a WordPress/CMS-specific supplement (plugin/theme CVEs, XML-RPC abuse, REST API `permission_callback` omission — none present in source, entirely original).
...
- DR-01.9 Full Cornucopia STRIDE EoP v5.0 cards (SP, TA, RE, ID, DS, EP suits).
- DR-01.10 Full Cornucopia MLSec v1.0 cards (EMR, EIR, EOR, EDR suits).
- DR-01.11 Full Digital-by-Default Harms v1.0 cards (SCO, ARC, AGE, TRU, POR, COR, WC suits), each with a reviewed Polish translation and a curated cross-reference row to OWASP A04:2021 Insecure Design.

### DR-02 — YAML Source-of-Truth and Curation Files
- DR-02.1 Cornucopia cards are maintained as YAML files in `data/cornucopia/`; severity/card_kind/cross-reference values are maintained separately in `data/cornucopia/curation/*.curation.json` (`PLAN.md` §0.1, §12).
- DR-02.2 `Card_Loader` loads all six YAML files and their matching curation files at plugin activation and on `Reingest_Deck_Job` execution; no live-reload from a REST-triggered path.
- DR-02.3 Every card row stores `content_sha256` (of the raw YAML card only), verified against `data/hashes.json` before being marked valid.

### DR-03 — Database Migrations
- DR-03.1 All schema changes are managed via `dbDelta()`-compatible `CREATE TABLE`/`ALTER TABLE` statements versioned alongside the plugin, run on activation and on version-upgrade detection.
- DR-03.2 Migrations are idempotent and forward-only; no destructive `DROP` in production migration paths.
...
| AC-07 | Fake OWASP reference injection via a curation-file PR | `Reference_Validator` + allowlist, run in CI against every `curation/*.curation.json` |
| AC-08 | Fake MITRE T-code injection | `Reference_Validator` + allowlist |
| AC-09 | Privilege escalation to `/stride-heatmap/` | `current_user_can()` capability check |
...
| AC-14 | REST route registered without a `permission_callback` | CI check grepping every `register_rest_route()` call for a present `permission_callback` argument |
...
| AC-17 | `wp-login.php` credential stuffing | Login-attempt rate limiting (SR-05.4) |
| AC-18 | XML-RPC pingback amplification/enumeration abuse | `xmlrpc.php` disabled entirely (SR-14.1) |
| AC-19 | A curation file silently invents a `severity`/reference for a card that doesn't exist in the raw YAML (typo or stale `card_id`) | Ingestion fails closed: a `card_id` present in a curation file but absent from its matching raw YAML aborts that deck's ingestion (SR-09.2) |
...
| FR-05 | US-05 | SR-07 | US-05 Playwright |
...
| SR-09 | — | D-08 | `eris`: unknown-field rejection property (raw + curation decoders), AC-19 |
...
| Nonce (WordPress) | A one-time, context-bound token used as WordPress's native CSRF defense |
...
| WPCS (WordPress Coding Standards) | The official PHPCS ruleset for WordPress code, including security-specific sniffs |
...
| Progressive enhancement | Building a page to be fully functional with server-rendered HTML alone, then layering JavaScript on top as an enhancement rather than a requirement |
| Curation file | A hand-authored, PR-reviewed JSON file (`curation/*.curation.json`) supplying `severity`/`card_kind`/`owasp_refs`/`mitre_refs` for a Cornucopia card, since the raw source YAML carries none of these fields |
